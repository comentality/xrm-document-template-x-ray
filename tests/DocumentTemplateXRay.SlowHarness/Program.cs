...
namespace DocumentTemplateXRay.SlowHarness
{
    ...
    ///
    /// Every scenario gets a window and a shot per beat of its own, and prints one line per
    /// finding. The exit code is the number of scenarios that had any.
    ///
    /// It hosts the real control, but hands it a <see cref="SlowService"/> through the connection
    /// property XrmToolBox would have set, so the tool runs for real: a template is a real .docx
    /// unzipped by the extractor it ships, and resolving its display names costs a round trip per
    /// table here the way it does against a real environment.
    /// </summary>
    internal static class Program
    {
        ...
        private static int Main(string[] args)
        {
            ...
            foreach (var scenario in scenarios)
            {
                ...
                Console.WriteLine(scenario.Name.PadRight(26) + scenario.Why);
